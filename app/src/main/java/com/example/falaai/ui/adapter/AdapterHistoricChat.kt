...
class AdapterHistoricChat(
    ...
    private var longClickChat: ((tracked: ModelChat) -> Unit)? = null

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding = ItemChatBinding.inflate(LayoutInflater.from(context), parent, false)
        return ViewHolder(context, clickChat, longClickChat, binding)
    }
    ...
    fun setOnClickChat(clickChat: (chat: ModelChat) -> Unit) {
        this.clickChat = clickChat
    }

    fun setOnLongClickChat(longClickChat: (chat: ModelChat) -> Unit) {
        this.longClickChat = longClickChat
    }
    ...
    inner class ViewHolder(
        ...
        fun bindView(modelChat: ModelChat, position: Int) {
            ...
            itemChatView.setOnLongClickListener {
                longClickChat?.let { it(modelChat) }
                true
            }
